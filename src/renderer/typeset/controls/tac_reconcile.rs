        ...
        if let Control::Table(t) = c {
            if flow.is_effective_tac_table(para, t, fmt) {
                if let Some(seg) = para.line_segs.get(tac_idx) {
                    ...
                    // 상류 절반 규칙 그대로 — 단 rhwp 가 다시 조판한 줄과 섞인 구역(채움·편집으로 사다리가
                    // 고쳐진 구역)의 저장 줄도 전부 센다(`TacFlowQuery::counts_full_tac_gap`). 흐름에 이미 실은
                    // 간격을 lazy 기준 역산이 다시 잇지 않게 `height_cursor` 가 같은 조건으로 다리를 끈다.
                    let ls_px = hwpunit_to_px(seg.line_spacing, dpi);
                    let ls_part = if flow.counts_full_tac_gap(para) {
                        ls_px
                    } else {
                        ls_px / 2.0
                    };
